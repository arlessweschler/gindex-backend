		...
		if(jwtVerify(req.headers.token)){
			User.findOne({ email: req.body.email }, function(error, result){
				if(result){
					...
					if(parsed.season){
						axios.get(`${searchPoint.tv}?api_key=${process.env.TMDBAPI}&page=1&query=${encodeURI(parsed.title)}&include_adult=false`).then(response => {
							if(response.data.results.length < 1){
								...
								axios.get(`${dataEndPoint.tv}/${response.data.results[0].id}?api_key=${process.env.TMDBAPI}&append_to_response=videos,images,reviews,people,networks,companies,collections,credits`).then(response => {
									...
									if(resp.revenue){
										resp.revenue = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(resp.revenue);
									}
									if(resp.budget){
										resp.budget = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(resp.budget);
									}
									if(resp.seasons){
									...
									res.status(200).send({ auth: true, registered: true, data: true, result: resp });
								})
							}
						})
					} else {
						axios.get(`${searchPoint.movie}?api_key=${process.env.TMDBAPI}&query=${encodeURI(parsed.title)}&page=1&include_adult=false&primary_release_year=${parsed.year}`).then(response => {
							if(response.data.results.length < 1){
								res.status(200).send({ auth: true, registered: true, data: false });
							} else {
